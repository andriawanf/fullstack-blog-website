...
const navigation = [
    ...
    { name: 'Discover Blogs', to: '/blog', icon: 'ri-blogger-line ri-xl' },
    { name: 'About Us', to: '#', icon: 'ri-team-line ri-xl' },
    ...
    return (
        ...
        <nav className="fixed left-0 right-0 z-10 flex items-center justify-between px-6 py-4 mx-4 top-4 bg-white-500/5 rounded-xl backdrop-blur-xl lg:mx-20 md:mx-10">
            ...
            <div className="items-center hidden gap-1 text-sm font-normal leading-relaxed xl:flex-row xl:flex font-nunito xl:pr-10">
                {navigation.map((menu) => (
                    <Link key={menu.name} to={menu.to} className={location.pathname === menu.to ? "flex items-center gap-2 px-4 py-2 rounded-lg text-white-500 bg-blueDark-500" : "flex items-center gap-2 px-4 py-2 rounded-lg text-white-700 hover:bg-blueDark-500/20 hover:text-white-500"}>
                        <i className={menu.icon}></i>
                        <p>{menu.name}</p>
                        {menu.name === "Discover Blogs" ? (<i className="ri-arrow-down-s-line ri-lg"></i>) : ""}
                    </Link>
                ))}
            </div>
            {/* Right Navbar */}
                ...
                <div>
                    {currentUser ? (
                        <Menu as="div" className="relative">
                            <div>
                                <Menu.Button className="relative hidden text-sm rounded-full bg-blueDark-800 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-blueDark-800 xl:flex">
                                    <span className="absolute -inset-1.5" />
                                    <span className="sr-only">Open user menu</span>
                                    <img
                                        className="object-cover w-10 h-10 rounded-full"
                                ...
                                <Menu.Button className="relative flex text-sm focus:outline-none xl:hidden">
                                    <span className="absolute -inset-1.5" />
                                    <span className="sr-only">Open user menu</span>
                                    <i className="mt-1 ri-menu-fill ri-lg text-white-500"></i>
                                </Menu.Button>
                            </div>
                            <Transition
                                as={Fragment}
                                ...
                                leaveTo="transform opacity-0 scale-95"
                            >
                                <Menu.Items className="absolute right-0 z-50 w-64 px-2 py-2 mt-2 space-y-3 origin-top-right shadow-lg rounded-xl bg-white-500 ring-1 ring-black ring-opacity-5 focus:outline-none">
                                    <Menu.Item>
                                        {({ active }) => (
                                            <a
                                                href="#"
                                                className={classNames(active ? 'bg-blueDark-500 text-white-500' : '', "flex items-center gap-4 px-2 py-2 rounded-lg")}>
                                                <img
                                                    className="object-cover w-10 h-10 rounded-full"
                                                    src={currentUser.data.profilePicture} alt={currentUser.data.name} />
                                                <div
                                                    href="#"
                                                    className='flex flex-col text-sm font-nunito'
                                                >
                                                    <p className="font-bold">{currentUser.data.username}</p>
                                                    <p className="font-normal text-white-700">{currentUser.data.email}</p>
                                                </div>
                                            </a>
                                        )}
                                    </Menu.Item>
                                    <div className="h-px rounded-full bg-black-500/10"></div>
                                    <div className="flex flex-col gap-2">
                                        <div className="flex flex-col gap-2 xl:hidden">
                                            <Menu.Item>
                                                {({ active }) => (
                                                    <a href="#" className={classNames(active ? 'bg-blueDark-500 text-white-500' : '', "flex items-center gap-2 px-4 py-2.5 rounded-lg")}>
                                            ...
                                            <Menu.Item>
                                                {({ active }) => (
                                                    <a href="#" className={classNames(active ? 'bg-blueDark-500 text-white-500' : '', "flex items-center gap-2 px-4 py-2.5 rounded-lg")}>
                                                        <i className="ri-blogger-line ri-lg"></i>
                                                        <div
                                                            className='text-sm font-semibold font-nunito'
                                                        >
                                                            <p>Discover Blogs</p>
                                            ...
                                            <Menu.Item>
                                                {({ active }) => (
                                                    <a href="#" className={classNames(active ? 'bg-blueDark-500 text-white-500' : '', "flex items-center gap-2 px-4 py-2.5 rounded-lg")}>
                                                        <i className="ri-contacts-line ri-lg"></i>
                                                        <div
                                                            className='text-sm font-semibold font-nunito'
                                                        >
                                                            <p>Contact Us</p>
                                        ...
                                        <Menu.Item>
                                            {({ active }) => (
                                                <a
                                                    href="#"
                                                    className={classNames(active ? 'bg-blueDark-500 text-white-500' : '', "flex items-center gap-2 px-4 py-2.5 rounded-lg")}>
                                    ...
                                    <Menu.Item>
                                        {({ active }) => (
                                            <button className="flex items-center justify-center px-4 py-2.5 rounded-lg bg-red-900 text-white-500 w-full" onClick={handleSignOut}>
                                                <div
                                                    className='text-sm font-medium font-nunito'
                                                >
                                                    <p>Logout</p>
                                                </div>
                                            </button>
